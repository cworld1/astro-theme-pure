...
<Aside type='tip'>
  To build using Docker manually, please refer to [Astro: Build your Astro site with
  Docker](https://docs.astro.build/en/recipes/docker/).
</Aside>
...
<Aside type='tip'>
  Running `--build` again does not necessarily rebuild every layer from scratch. Docker reuses
  cached layers when possible, so unchanged dependencies and other build steps can still be cached.
</Aside>
